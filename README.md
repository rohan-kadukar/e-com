...
Scripts to use:
- dev: `next dev` [web:60][web:57]
- postinstall: `prisma generate` [web:22][web:57]
- build: `next build` [web:60][web:57]
- start: `next start` [web:60][web:57]

Workflow:
- After `npm install`, Prisma Client is generated automatically via postinstall. [web:22][web:57]
- When you edit `schema.prisma`, run: `npx prisma generate`, then `npm run dev`. [web:57][web:60]

## Windows Defender exclusions

Prevent file locking (EPERM) on Windows:
- Open Windows Security → Virus & threat protection → Manage settings → Exclusions → Add or remove exclusions. [web:49][web:41]
- Add your project folder: `D:\SparkLabIT\Next JS\ECOM`. [web:49][web:41]
- Add your npm cache folder as an exclusion as well. [web:49][web:41]

## Why these changes

- Running `prisma generate` inside the `dev` command can clash with a live Next.js process and cause Windows rename/permission errors; moving generate to postinstall and running it manually after schema edits avoids locks. [web:22][web:57]
