...
const Header = () => {
  ...
  const rows = await response.json(); // [{id, userId, productId}]

  // Store expects id equal to product.id used in UI comparisons
  const productArray = rows.map((row: any) => ({ id: String(row.productId) }));
